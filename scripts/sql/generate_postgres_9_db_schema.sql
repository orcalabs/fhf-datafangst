...
create table optimized_catch_data
(
...
create table reduced_catch_data
(
...
create unique index reduced_catch_data_pk_reduced_catch_data_uindex
	on reduced_catch_data (pk_reduced_catch_data);

create index reduced_catch_data_detailed_query_index
	on reduced_catch_data (year_and_month, fangstfelt, art_kode, lengdekode, kvalitetkode, redskap_kode);

create table raw_catch_data
(
	index bigint,
	dokumentnummer bigint,
...
create index ix_raw_catch_data_index
	on raw_catch_data (index);

create unique index raw_catch_data_pk_raw_catch_data_uindex
...
create trigger suppress_redundant_raw_updates
	...
	on raw_catch_data
	for each row
	execute procedure pg_catalog.suppress_redundant_updates_trigger();

create table raw_catch_data_staging_table
(
	index bigint,
	dokumentnummer bigint,
	dokumenttype_kode bigint,
...
create table monthly_aggregated_grouped_catch_data
(
	rundvekt integer,
	fangstfelt text,
	art text,
...
create index monthly_aggregated_grouped_catch_data_year_month_index
...
create table monthly_aggregated_grouped_catch_data_staging_table
(
	rundvekt integer,
	fangstfelt text,
	art text,
...
create table monthly_averaged_temperature_and_air_pressure_staging_table
(
	datetime timestamp,
	catch_area_id text,
...
create table monthly_averaged_temperature_and_air_pressure
(
	pk serial not null
...
create unique index monthly_averaged_temperature_and_air_pressure_by_catch_area_pk_
	on monthly_averaged_temperature_and_air_pressure (pk);

create unique index monthly_averaged_temperature_and_air_pressure_by_catch_area_dat
...
create table flagstad_received_catch
(
	datetime_landing timestamp with time zone,
	index bigint,
	dokumentnummer bigint,
	dokumenttype_kode bigint,
	dokumenttype text,
	...
	oppdateringstidspunkt text,
	lok text,
	pk_raw_catch_data bigint
);

alter table flagstad_received_catch owner to postgres;

create table flagstad_catch_data
(
	index bigint,
	dokumentnummer bigint,
	dokumenttype_kode bigint,
...
alter table flagstad_catch_data owner to postgres;

create function add_new_data_to_raw_catch_data_table() returns trigger
	language plpgsql
as $$
BEGIN
...
create function propagate_data_to_reduced_catch_data_table() returns trigger
	language plpgsql
as $$
BEGIN
    /*
...
create function generate_monthly_aggregated_catch_data() returns trigger
	language plpgsql
as $$
BEGIN
    /*
...
create FUNCTION  aggregate_monthly_catch_data()
    RETURNS RECORD
	language plpgsql
as $$
BEGIN
...
alter FUNCTION aggregate_monthly_catch_data() owner to postgres;

create FUNCTION usp_truncate_catch_data_staging_table()
    RETURNS RECORD
	language plpgsql
as $$
BEGIN
...
alter FUNCTION usp_truncate_catch_data_staging_table() owner to postgres;

create function upsert_monthly_aggregated_catch_records_from_staging_table()
    returns trigger
	language plpgsql
as $$
BEGIN
...
create FUNCTION usp_truncate_temperature_and_air_pressure_staging_table()
    returns trigger
	language plpgsql
as $$
BEGIN
...
alter FUNCTION usp_truncate_temperature_and_air_pressure_staging_table() owner to postgres;

create function add_new_data_to_weather_data_table()
    returns trigger
	language plpgsql
as $$
BEGIN
...
create FUNCTION usp_update_monthly_catch_table_with_weather_values()
    returns trigger
	language plpgsql
as $$
BEGIN
...
alter FUNCTION usp_update_monthly_catch_table_with_weather_values() owner to postgres;
